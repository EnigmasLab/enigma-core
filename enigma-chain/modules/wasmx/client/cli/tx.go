...
func NewContractRegistrationRequestProposalTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Submit a proposal to register contract.
			Example:
			$ %s tx xwasm propose-contract-registration-request --contract-gas-limit 20000 --contract-gas-price "1000000000" --contract-address "fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9swvf72y" (--contract-funding-mode=dual) (--granter-address=fury1dzqd00lfd4y4qy2pxa0dsdwzfnmsu27hgttswz) --pin-contract=true --from mykey
		`,
	...
	cmd.Flags().Uint64(FlagContractGasPrice, 1000000000, "gas price in fury to use for the contract execution")
...
func NewContractDeregistrationRequestProposalTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Submit a proposal to deregister contract.
			Example:
			$ %s tx xwasm propose-contract-registration-request --contract-gas-limit 20000 --contract-gas-price "1000000000" --code-id 1 --contract-address "fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9swvf72y"  --granter-address=fury1dzqd00lfd4y4qy2pxa0dsdwzfnmsu27hgttswz --contract-funding-mode self-funded --pin-contract=true --from wasm --chain-id enigma-1 
		`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func ContractParamsUpdateTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Update registered contract params (gas price, gas limit, admin address).
			Example:
			$ %s tx xwasm contract-params-update --contract-gas-limit 20000 --contract-gas-price "1000000000" --contract-address "fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9swvf72y" --contract-admin="fury1p7z8p649xspcey7wp5e4leqf7wa39kjjj6wja8" --from mykey
		`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func ContractActivateTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Activate registered contract to be executed in begin blocker.
			Example:
			$ %s tx xwasm contract-activate --contract-address "fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9swvf72y" --from mykey
		`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func ContractDeactivateTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Deactivate registered contract (will no longer be executed in begin blocker, but remains registered).
			Example:
			$ %s tx xwasm contract-deactivate --contract-address "fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9swvf72y" --from mykey
		`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func RegisterContractTxCmd() *cobra.Command {
	...
	cmd.Example = "enigmad tx xwasm register-contract fury1apapy3g66m52mmt2wkyjm6hpyd563t90u0dgmx 4500000 1000000000 true true 1 fury17gkuet8f6pssxd8nycm3qr9d9y699rupv6397z"
