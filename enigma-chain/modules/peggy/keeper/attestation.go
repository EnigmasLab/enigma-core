...
func (k *Keeper) ProcessClaimData(ctx sdk.Context, claim types.EthereumClaim) {
	...
	case *types.MsgDepositClaim:
		...
		if claim.Data != "" {
			...
			ethereumSenderFuryAccAddr := sdk.AccAddress(common.FromHex(claim.EthereumSender))
			claimDataMsg, err := k.ValidateClaimData(ctx, claim.Data, ethereumSenderFuryAccAddr)
			if err != nil {
