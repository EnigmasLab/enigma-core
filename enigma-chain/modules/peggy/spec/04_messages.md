...
This is a reference document for Peggy message types. For code reference and exact arguments see the [proto definitions](https://github.com/EnigmasLab/enigma-core/blob/master/proto/enigma/peggy/v1/msgs.proto). 

## User messages

These are messages sent on the Enigma Chain peggy module. See [workflow](./02_workflow.md) for a more detailed summary of the entire deposit and withdraw process.
...
// single coin not a set of coins that is normal in other Enigma messages
// FEE:
...
to pay the relayer. Note that this transaction will contain two fees. One fee amount to submit to the Enigma Chain, that can be paid
...
These are messages run by relayers. Relayers are unpermissioned and simply work to move things from the Enigma Chain to Ethereum.
...
Relayers use `QueryPendingSendToEth` in [query.proto](https://github.com/EnigmasLab/enigma-core/blob/master/proto/enigma/peggy/v1/query.proto) to query the potential fees for a batch of each
...
All validators run two processes in addition to their Enigma node. An Ethereum oracle and Ethereum signer, these are bundled into a single Orchestrator binary for ease of use.

The oracle observes the Ethereum chain for events from the [Peggy.sol](https://github.com/EnigmasLab/peggo/blob/master/solidity/contracts/Peggy.sol) contract before submitting them as messages to the Enigma Chain.
...
// issued to the Enigma address in question
// -------------
...
```
Deposit claims represent a `SendToCosmosEvent` emitted by the Peggy contract. After 2/3 of the validators confirm a deposit claim,  the representative tokens will be issued to the specified `CosmosReceiver` Enigma Chain account.
...
All validators run two processes in addition to their Enigma Chain node. An Ethereum oracle and Ethereum signer, these are bundled into a single Orchestrator binary for ease of use.

The Ethereum signer watches several [query endpoints](https://github.com/EnigmasLab/enigma-core/blob/master/proto/enigma/peggy/v1/query.proto) and it's only job is to submit a signature for anything that appears on those endpoints. For this reason the validator must provide a secure RPC to an Enigma Chain node following chain consensus. Or they risk being tricked into signing the wrong thing.
...
// VALIDATOR
// The validator field is a furyvaloper1... string (i.e. sdk.ValAddress)
// that references a validator in the active set
// ORCHESTRATOR
// The orchestrator field is a fury1... string  (i.e. sdk.AccAddress) that
