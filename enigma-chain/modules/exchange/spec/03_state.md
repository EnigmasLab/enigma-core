...
```go
type Params struct {
	// spot_market_instant_listing_fee defines the expedited fee in FURY required to create a spot market by bypassing governance
	SpotMarketInstantListingFee types.Coin
	// derivative_market_instant_listing_fee defines the expedited fee in FURY required to create a derivative market by bypassing governance
	...
	// fury_reward_staked_requirement_threshold defines the threshold on FURY rewards after which one also needs staked FURY to receive more
	FuryRewardStakedRequirementThreshold github_com_cosmos_cosmos_sdk_types.Int
...
```go
// An object describing a derivative market in the Enigma Futures Protocol.
