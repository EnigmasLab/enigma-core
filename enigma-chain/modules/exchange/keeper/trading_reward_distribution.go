...
func (k *Keeper) distributeTradingRewardsForAccount(
	...
	furyRewardStakedRequirementThreshold := k.GetFuryRewardStakedRequirementThreshold(ctx)
	...
	for _, coin := range maxCampaignRewards {
		...
		if coin.Denom == chaintypes.EnigmaCoin && accountRewardAmount.GT(furyRewardStakedRequirementThreshold) {
			stakedFURY := k.CalculateStakedAmountWithoutCache(ctx, accountPoints.Account)
			minRewardAboveThreshold := furyRewardStakedRequirementThreshold

			// at least X amount of FURY (e.g. 100 FURY), but otherwise not more than the staked amount
			accountRewardAmount = sdk.MaxInt(minRewardAboveThreshold, sdk.MinInt(accountRewardAmount, stakedFURY))
		}
