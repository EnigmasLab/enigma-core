...
func (k *Keeper) WithdrawAllAuctionBalances(ctx sdk.Context) []SendToAuctionCoin {
	...
	for _, balance := range balances {
		...
		if balance.Denom == chaintypes.EnigmaCoin {
			furyBurnAmount := sdk.NewCoins(sdk.NewCoin(chaintypes.EnigmaCoin, balance.Amount))
			if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, furyBurnAmount); err != nil {
