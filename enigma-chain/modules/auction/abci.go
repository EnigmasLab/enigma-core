...
func (am AppModule) EndBlocker(ctx sdk.Context, block abci.RequestEndBlock) {
	...
	if lastBidAmount.IsPositive() && lastBid.Bidder != "" {
		...
		for _, coin := range coins {
			// burn exactly module's fury amount received from bid
			if coin.Denom == chaintypes.EnigmaCoin {
				furyBurnAmount := sdk.NewCoins(sdk.NewCoin(chaintypes.EnigmaCoin, lastBidAmount))
				err = am.bankKeeper.BurnCoins(ctx, auctiontypes.ModuleName, furyBurnAmount)
				if err != nil {
					metrics.ReportFuncError(am.svcTags)
					logger.Info(err.Error())
				}
