...
// Setup initializes a new EnigmaApp. A Nop logger is set in EnigmaApp.
func Setup(isCheckTx bool) *EnigmaApp {
	sdk.DefaultBondDenom = "fury"

	db := dbm.NewMemDB()
	app := NewEnigmaApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 1, MakeEncodingConfig(), TestAppOptions{})
