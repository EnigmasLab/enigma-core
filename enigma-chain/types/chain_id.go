...
var (
	...
	regexSeparator   = `-{1}`
	regexEpoch       = `[1-9][0-9]*`
	enigmaChainID = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)$`, regexChainID, regexSeparator, regexEpoch))
)
...
func IsValidChainID(chainID string) bool {
	...
	return enigmaChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := enigmaChainID.FindStringSubmatch(chainID)
...
func GenerateRandomChainID() string {
	return fmt.Sprintf("enigma-%d", 10+tmrand.Intn(10000))
}
