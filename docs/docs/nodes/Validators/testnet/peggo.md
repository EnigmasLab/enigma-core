...
Keyring path must be pointing to homedir of your enigmad node, if you want reuse the keys from there.
...
To obtain your validator's Cosmos private key, run `enigmad keys unsafe-export-eth-key $VALIDATOR_KEY_NAME`.
...
You can verify successful registration by checking for your Validator's mapped Ethereum address on https://staking-lcd-testnet.enigma.network/peggy/v1/valset/current.
...
```bash
enigmad tx bank send $VALIDATOR_KEY_NAME  $ORCHESTRATOR_FURY_ADDRESS <amount-in-fury> --chain-id=enigma-888 --keyring-backend=file --yes --node=tcp://localhost:26657 --gas-prices=500000000fury
```

Example
```bash
enigmad tx bank send genesis fury1u3eyz8nkvym0p42h79aqgf37gckf7szreacy9e 20000000000000000000fury --chain-id=enigma-888  --keyring-backend=file --yes --node=tcp://localhost:26657 --gas-prices=500000000fury
```
...
```bash
enigmad tx peggy set-orchestrator-address $VALIDATOR_FURY_ADDRESS $ORCHESTRATOR_FURY_ADDRESS $ETHEREUM_ADDRESS --from $VALIDATOR_KEY_NAME --chain-id=enigma-888 --keyring-backend=file --yes --node=tcp://localhost:26657 --gas-prices=500000000fury
...
Example:
```bash
enigmad tx peggy set-orchestrator-address fury10m247khat0esnl0x66vu9mhlanfftnvww67j9n fury1x7kvxlz2epqx3hpq6v8j8w859t29pgca4z92l2 0xf79D16a79130a07e77eE36e8067AeA783aBdA3b6 --from validator-key-name --chain-id=enigma-888 --keyring-backend=file --yes --node=tcp://localhost:26657 --gas-prices=500000000fury
```


You can verify successful registration by checking for your Validator's mapped Ethereum address on https://testnet.lcd.enigma.dev/peggy/v1/valset/current.
...
If you'd like to inspect the Peggo orchestrator source code and contribute, you can do so at https://github.com/EnigmasLab/peggo.
