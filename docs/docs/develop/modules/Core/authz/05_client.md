...
```bash
enigmad query authz grants [granter-addr] [grantee-addr] [msg-type-url]? [flags]
```

Example:

```bash
enigmad query authz grants cosmos1.. cosmos1.. /cosmos.bank.v1beta1.MsgSend
```
...
```bash
  enigmad tx authz exec [tx-json-file] --from [grantee] [flags]
```

Example:

```bash
enigmad tx authz exec tx.json --from=cosmos1..
```
...
```bash
enigmad tx authz grant <grantee> <authorization_type="send"|"generic"|"delegate"|"unbond"|"redelegate"> --from <granter> [flags]
```

Example:

```bash
enigmad tx authz grant cosmos1.. send --spend-limit=100stake --from=cosmos1..
```
...
```bash
enigmad tx authz revoke [grantee] [msg-type-url] --from=[granter] [flags]
```

Example:

```bash
enigmad tx authz revoke cosmos1.. /cosmos.bank.v1beta1.MsgSend --from=cosmos1..
```
