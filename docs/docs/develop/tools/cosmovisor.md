...
* `DAEMON_HOME` is the location where upgrade binaries should be kept (e.g. `$HOME/.enigmad`).
* `DAEMON_NAME` is the name of the binary itself (eg. `enigmad`).
...
program will store it's data under their default data directory (e.g. `$HOME/.enigmad`) which is independent of
...
The following instructions provide a demonstration of `cosmovisor`'s integration with the `enigmad` application
shipped along the Cosmos SDK's source code.

First compile `enigmad`:
...
Create a new key and setup the `enigmad` node:

```
rm -rf $HOME/.enigmad
./build/enigmad keys --keyring-backend=file add validator
./build/enigmad init testing --chain-id=enigma-1
./build/enigmad add-genesis-account --keyring-backend=file $(./build/enigmad keys --keyring-backend=file show validator -a) 1000000000fury
./build/enigmad gentx --keyring-backend=file --chain-id=enigma-1 validator 100000fury
./build/enigmad collect-gentxs
```

Set the required environment variables:

```
export DAEMON_NAME=enigmad         # binary name
export DAEMON_HOME=$HOME/.enigmad  # daemon's home directory
```
...
cp ./build/enigmad $DAEMON_HOME/cosmovisor/genesis/bin
```

For the sake of this demonstration, we would amend `voting_params.voting_period` in `.enigmad/config/genesis.json` to a reduced time ~5 minutes (300s) and eventually launch `cosmosvisor`:
...
```
./build/enigmad tx gov submit-proposal software-upgrade test1 --title="upgrade-demo" --description="upgrade"  --from=validator --upgrade-height=100 --deposit=10000000fury --chain-id=enigma-1 --keyring-backend=test -y
```

Query the proposal to ensure it was correctly broadcast and added to a block:

```
./build/enigmad query gov proposal 1
```

Submit a `Yes` vote for the upgrade proposal:

```
./build/enigmad tx gov vote 1 yes --from=validator --keyring-backend=file --chain-id=enigma-1 -y
```
